...
# ###############################################################
# ## Device Selection Policy (Payment Service)
# ## [tenant->Services->L4-L7->Device Selection Policies]
# ###############################################################
# resource "aci_logical_device_context" "payment_service" {
#   tenant_dn         = data.aci_tenant.user_tenant.id
#   ctrct_name_or_lbl = aci_contract.permit_to_online_boutique_payment_service_contract.name
...
# }
# resource "aci_logical_interface_context" "payment_service_consumer" {
#   logical_device_context_dn  = aci_logical_device_context.payment_service.id
...
# }
# resource "aci_logical_interface_context" "payment_service_provider" {
#   logical_device_context_dn  = aci_logical_device_context.payment_service.id
...
## [tenant->Contracts->Standard->permit-to-online-boutique-payment-service->redirect]
#####################################################################################
# resource "aci_rest_managed" "enable_payment_svc_service_graph" {
#   dn         = "${aci_contract_subject.permit_to_online_boutique_payment_service_contract_subject.id}/rsSubjGraphAtt"
...
#   depends_on = [
#     aci_logical_device_context.payment_service,
#     aci_logical_interface_context.payment_service_consumer,
#     aci_logical_interface_context.payment_service_provider
#   ]
# }

###############################################################
## Device Selection Policy (Frontend Service)
## [tenant->Services->L4-L7->Device Selection Policies]
###############################################################
resource "aci_logical_device_context" "frontend_service" {
  tenant_dn         = data.aci_tenant.user_tenant.id
  ctrct_name_or_lbl = aci_contract.permit_to_online_boutique_frontend_service_contract.name
  graph_name_or_lbl = aci_l4_l7_service_graph_template.service_graph_template.name
  node_name_or_lbl  = aci_function_node.service_graph_template_function_node.name
  relation_vns_rs_l_dev_ctx_to_l_dev = "${data.aci_tenant.user_tenant.id}/lDevIf-[${data.aci_l4_l7_device.ftdv_device.id}]"
}
resource "aci_logical_interface_context" "frontend_service_consumer" {
  logical_device_context_dn  = aci_logical_device_context.frontend_service.id
  conn_name_or_lbl  = "consumer"
  l3_dest  = "no"
  permit_log  = "no"
  relation_vns_rs_l_if_ctx_to_svc_redirect_pol = aci_service_redirect_policy.pbr_policy.id
...
resource "aci_logical_interface_context" "frontend_service_provider" {
  logical_device_context_dn  = aci_logical_device_context.frontend_service.id
  conn_name_or_lbl  = "provider"
  l3_dest  = "no"
  permit_log  = "no"
...
####################################################################################
# Enable Contract redirect to Service Graph
# [tenant->Contracts->Standard->permit-to-online-boutique-frontend-service->redirect]
####################################################################################
resource "aci_rest_managed" "enable_frontend_svc_service_graph" {
  dn         = "${aci_contract_subject.permit_to_online_boutique_frontend_service_contract_subject.id}/rsSubjGraphAtt"
  class_name = "vzRsSubjGraphAtt"
  content = {
    tnVnsAbsGraphName = aci_l4_l7_service_graph_template.service_graph_template.name
  }
  ...
  depends_on = [
    aci_logical_device_context.frontend_service,
    aci_logical_interface_context.frontend_service_consumer,
    aci_logical_interface_context.frontend_service_provider
  ]
}

###############################################################
## Device Selection Policy (Redis Database)
## [tenant->Services->L4-L7->Device Selection Policies]
###############################################################
resource "aci_logical_device_context" "redis_database" {
  tenant_dn         = data.aci_tenant.user_tenant.id
  ctrct_name_or_lbl = aci_contract.permit_to_online_boutique_redis_database_contract.name
  graph_name_or_lbl = aci_l4_l7_service_graph_template.service_graph_template.name
  node_name_or_lbl  = aci_function_node.service_graph_template_function_node.name
  relation_vns_rs_l_dev_ctx_to_l_dev = "${data.aci_tenant.user_tenant.id}/lDevIf-[${data.aci_l4_l7_device.ftdv_device.id}]"
}
resource "aci_logical_interface_context" "redis_database_consumer" {
  logical_device_context_dn  = aci_logical_device_context.redis_database.id
  conn_name_or_lbl  = "consumer"
  l3_dest  = "no"
  permit_log  = "no"
  relation_vns_rs_l_if_ctx_to_svc_redirect_pol = aci_service_redirect_policy.pbr_policy.id
...
resource "aci_logical_interface_context" "redis_database_provider" {
  logical_device_context_dn  = aci_logical_device_context.redis_database.id
  conn_name_or_lbl  = "provider"
  l3_dest  = "no"
  permit_log  = "no"
...
####################################################################################
# Enable Contract redirect to Service Graph
# [tenant->Contracts->Standard->permit-to-online-boutique-redis_database-service->redirect]
####################################################################################
resource "aci_rest_managed" "enable_redis_database_svc_service_graph" {
  dn         = "${aci_contract_subject.permit_to_online_boutique_redis_database_contract_subject.id}/rsSubjGraphAtt"
  class_name = "vzRsSubjGraphAtt"
  content = {
    tnVnsAbsGraphName = aci_l4_l7_service_graph_template.service_graph_template.name
  }
  ...
  depends_on = [
    aci_logical_device_context.redis_database,
    aci_logical_interface_context.redis_database_consumer,
    aci_logical_interface_context.redis_database_provider
  ]
}
