...
resource "aci_endpoint_security_group" "databases_esg" {
  count = length(local.model.databases_app_profile.esgs)

  application_profile_dn = aci_application_profile.databases_app_profile.id
  name                   = local.model.databases_app_profile.esgs[count.index].name

  relation_fv_rs_scope   = data.aci_vrf.user_vrf.id

  dynamic "relation_fv_rs_prov" {
    for_each = local.model.databases_app_profile.esgs[count.index].relation_fv_rs_prov
    content {
      target_dn = relation_fv_rs_prov.value
    }
  }
  ...
  dynamic "relation_fv_rs_cons" {
    for_each = local.model.databases_app_profile.esgs[count.index].relation_fv_rs_cons
    content {
      target_dn = relation_fv_rs_cons.value
    }
  }
...
resource "aci_endpoint_security_group_tag_selector" "databases_tag_selector" {
  ...
  endpoint_security_group_dn = aci_endpoint_security_group.databases_esg[count.index].id
  match_key                  = local.model.databases_app_profile.esgs[count.index].key
  match_value                = local.model.databases_app_profile.esgs[count.index].value
  value_operator             = "equals"
}